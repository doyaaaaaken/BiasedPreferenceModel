...
private[boot] object LifeSpanAveCsvOutputter extends PrintWriterUser {
  ...
  override def csvOutput(pw: PrintWriter, rankLimit: Int): Unit = {
    ...
    println("全様式における、normal様式・hoped様式それぞれの様式寿命平均値出力完了")

    getAndOutputMaxFreqAverage(pw, con, Some(100))
    println("TOP100における、normal様式・hoped様式それぞれの様式寿命平均値出力完了")

    getAndOutputMaxFreqAverage(pw, con, Some(40))
    println("TOP40における、normal様式・hoped様式それぞれの様式寿命平均値出力完了")

    getAndOutputMaxFreqAverage(pw, con, Some(20))
    println("TOP20における、normal様式・hoped様式それぞれの様式寿命平均値出力完了")
  }
