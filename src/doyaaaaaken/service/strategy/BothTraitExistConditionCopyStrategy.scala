...
private[service] object BothTraitExistConditionCopyStrategy {
  ...
  private[service] def debugPrint(debug: Boolean, copyAgentNumList: Seq[(Agent, Int)], copyAgentComList: Seq[(Agent, Int, Double)], copyAgentInfoList: Seq[(Agent, Int, Int, Double)], traitCopyInfoList: Seq[(Agent, Int, Boolean)], preferenceCopyInfoList: Seq[(Agent, Int, Double)]): Unit = {
    if (debug) {
      println("""|＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
